...
def plot_d_svdd(data, model, sph_center, sph_radius, device):
    """
    Plots the D-SVDD result showing support vectors, bounded support vectors, and the
    ...
    model.eval()
    with torch.no_grad():
        phi_x = model(torch.tensor(data, dtype=torch.float32, device=device)).cpu().detach().numpy()
    ...
    is_sv = ((dist_to_center >= radius - 0.005) & (dist_to_center < radius))
