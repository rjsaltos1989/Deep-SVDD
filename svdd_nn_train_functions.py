...
def run_d_svdd_epoch(model, optimizer, data_loader, loss_func, sph_center, sph_radius, nu, d_svdd_type,
    ...
    for inputs, _ in data_loader:
        ...
        # Moves the inputs to the device
        inputs = inputs.to(device)
        ...
        # Compute loss.
        loss = loss_func(phi_x, sph_center, sph_radius=sph_radius, nu=nu, d_svdd_type=d_svdd_type)
...
def train_d_svdd_network(model, loss_func, train_loader,
                         nu=0.05, d_svdd_type='soft-boundary', init_lr=0.001, min_lr=0.0001,
    ...
    for epoch in tqdm(range(epochs), desc="Epoch"):
        ...
        total_train_time += run_d_svdd_epoch(model, optimizer, train_loader, loss_func, sph_center, sph_radius,
                                             nu, d_svdd_type, results, device, prefix="train")
